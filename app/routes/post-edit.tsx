...
export async function action({ request, context }: Route.ActionArgs) {
	...
	try {
		...
		if (error instanceof PostMutationError) {
			const messages = { "not-found": "That post is no longer available.", "forbidden": "You cannot edit that post.", "organization-required": "Choose an organization for that visibility setting.", "organization-unavailable": "You cannot post for that organization.", "affiliation-required": "Choose at least one affiliation, or opt the posting organization in to the NH Connect public directory for statewide sharing.", "affiliation-unavailable": "You can only tag affiliations you belong to.", "event-details-required": "Add the event date and time before submitting it." };
...
export function meta({ data }: Route.MetaArgs) {
	return [{ title: `Edit ${data?.post.title ?? "post"} · NH Connect` }];
}
