...
export function PostEditor({
	...
	const [visibility, setVisibility] = useState(isUpdate ? "members" : post?.visibility ?? "members");
	return (
		<Form className="panel post-editor-form" method="post">
			...
			<div className="post-editor-grid">
				...
				{isUpdate ? <input name="visibility" type="hidden" value="members" /> : <label>Visibility<select name="visibility" onChange={(event) => setVisibility(event.currentTarget.value as typeof visibility)} value={visibility}>{postVisibilities.map((option) => <option key={option} value={option}>{visibilityLabels[option]}</option>)}</select></label>}
