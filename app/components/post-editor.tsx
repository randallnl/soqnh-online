...
import { imageUploadAccept, MAX_PROJECT_PHOTOS, mediaUrl } from "~/lib/media";
	...
	return (
		<Form className="panel post-editor-form" encType="multipart/form-data" method="post">
			...
			<div className="post-editor-grid">
				...
				{(isUpdate || section === "projects") && <label className="wide-field">{section === "projects" ? post ? "Add a photo" : "Project photos" : "Image"} <span>(optional)</span><input accept={imageUploadAccept} multiple={section === "projects" && !post} name="image" type="file" /><span className="field-help">{section === "projects" && !post ? `Select up to ${MAX_PROJECT_PHOTOS} photos. ` : ""}PNG, JPG, WebP, or GIF. Maximum 10 MB per photo.</span></label>}
				...
				<label>Post as<select defaultValue={post?.organizationId ?? organizations[0]?.id ?? ""} name="organizationId"><option value="">Yourself</option>{organizations.map((organization) => <option key={organization.id} value={organization.id}>{organization.name}{organization.directoryStatus === "published" ? " · Public directory" : ""}</option>)}</select></label>
